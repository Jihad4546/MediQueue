    ...
    const fetchTutors = async () => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/myTutors/${userEmail}`
        );
        ...
        {tutors.map((tutor, index) => (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-4">
              <div>
                <p className="text-[10px] uppercase tracking-wide text-gray-400 dark:text-gray-500 font-medium mb-0.5">Category</p>
                <p className="text-sm text-gray-700 dark:text-gray-300">{tutor.category}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-wide text-gray-400 dark:text-gray-500 font-medium mb-0.5">Price</p>
                <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">৳{tutor.price}/hr</p>
              </div>
              <div className="col-span-2">
                <p className="text-[10px] uppercase tracking-wide text-gray-400 dark:text-gray-500 font-medium mb-0.5">Location</p>
                <p className="text-sm text-gray-700 dark:text-gray-300">{tutor.location}</p>
              </div>
            </div>
      ...
      {selectedTutor && (
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 px-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-5 sm:p-8 w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto border border-gray-100 dark:border-gray-800">
            ...
            <p className="text-sm text-gray-400 dark:text-gray-500 mb-5">Edit the details and save changes.</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setUpdateForm({ ...updateForm, teachingMode: e.target.value })}
                  className="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm focus:outline-none focus:border-blue-400"
                >
